use crate::{
    ...
    matching::{
        match_tuple_type_arguments, matches_params, params::has_overlapping_params, CallableLike,
        Match, Matcher, MismatchReason, Type,
    },
    node_ref::NodeRef,
    ...
    pub fn overlaps(&self, i_s: &InferenceState, other: &Self) -> bool {
        ...
        match self {
            ...
            DbType::Callable(c1) => match other {
                ...
                DbType::Type(t2) => Type::new(self).overlaps(i_s, &Type::new(t2)),
                _ => false,
            },
            DbType::Any => true,
            DbType::Never => todo!(),
        ...
        match self {
            ...
            DbType::Type(t1) => match value_type {
                DbType::Type(t2) => Type::new(t1)
                    .matches(i_s, matcher, t2, variance)
                    .similar_if_false(),
                _ => Match::new_false(),
            },
            DbType::TypeVar(t1) => {
    ...
    fn matches_union(
        ...
        match value_type {
            DbType::TypeVar(type_var2) if matcher.is_matching_reverse() => {
                matcher.match_or_add_type_var(i_s, type_var2, &Type::new(self), variance.invert())
            }
