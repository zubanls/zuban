...
impl<'db> PythonString<'db> {
    ...
    fn union(self, other: Self) -> Self {
        match other {
            Self::Ref(_, s2) => match self {
                Self::Ref(start, s1) => Self::String(start, s1.to_owned() + s2),
                Self::String(start, s1) => Self::String(start, s1 + s2),
                Self::FString => Self::FString,
            },
            Self::String(_, s2) => match self {
                Self::Ref(start, s1) => Self::String(start, s1.to_owned() + &s2),
                Self::String(start, s1) => Self::String(start, s1 + &s2),
                Self::FString => Self::FString,
            },
    ...
    pub fn into_cow_str(self) -> Option<Cow<'db, str>> {
        match self {
...
fn trim_string_in_place(s: &mut String) {
    let trimmed = s.trim();
    let start = s.find(trimmed).unwrap();
