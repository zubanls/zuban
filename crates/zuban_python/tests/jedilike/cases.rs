...
fn parse_python_list(input: &str) -> Vec<String> {
    // This function was generated by LLMs
    let mut result = vec![];
...
pub(crate) fn run_parse_python_list_tests() {
    let cases = [
        ("['foo', 'bar']", vec!["foo", "bar"]),
        ("['foo,bar', 'baz']", vec!["foo,bar", "baz"]),
        (r"['it\'s fine', 'test']", vec!["it's fine", "test"]),
        ("['', 'foo']", vec!["", "foo"]),
        (" [  'foo'  ,   'bar' ] ", vec!["foo", "bar"]),
        ("['foo', \"bar\"]", vec!["foo", "bar"]),
        ("[\"foo\", 'bar']", vec!["foo", "bar"]),
        ("['foo,bar', \"baz\"]", vec!["foo,bar", "baz"]),
        (r#"["it's fine", 'ok']"#, vec!["it's fine", "ok"]),
        (r#"['a "b" c', "d 'e' f"]"#, vec!["a \"b\" c", "d 'e' f"]),
        ("['x', \"y\", 'z']", vec!["x", "y", "z"]),
        ("[]", vec![]),
        // single escaped quote preceded by double backslash
        (r"['it\\\'s']", vec!["it\\'s"]),
    ...
    for (i, (input, expected)) in cases.into_iter().enumerate() {
        let actual = parse_python_list(input);
