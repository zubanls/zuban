...
macro_rules! pos {
    ($p:expr) => {
        Position {
            line: $p.line_zero_based() as u32,
            character: $p.utf16_code_units_column() as u32,
        }
    };
}
macro_rules! range {
...
impl LS {
    ...
    pub fn new(mypy_compat: bool) -> Result<LS, JsValue> {
        ...
        std::panic::catch_unwind(AssertUnwindSafe(|| {
            ...
            s.files_or_directories_to_check = vec![GlobAbsPath::new(
                &fs,
                &fs.unchecked_abs_path(""),
                "**/*.py",
            )?];
            let flags = if mypy_compat {
    ...
    pub fn handle_message(&mut self, msg: &str) -> Option<String> {
        std::panic::catch_unwind(AssertUnwindSafe(|| self.dispatch(msg))).unwrap_or_else(|p| {
            Some(self.err(
                None,
                -32603,
                &format!("Internal error: {}", panic_payload_to_string(p)),
            ))
        })
    }
    ...
    pub fn register_stubs(&mut self, stubs: JsValue) -> Result<u32, JsValue> {
        let stubs: HashMap<String, String> =
            serde_wasm_bindgen::from_value(stubs).map_err(|e| JsValue::from_str(&e.to_string()))?;
        let n = stubs.len() as u32;
        for (path, content) in stubs {
            self.fs.set_file(&path, content);
        }
        self.proj
            .add_site_packages(self.fs.normalize_unchecked_abs_path("site-packages"));
        Ok(n)
    }
    ...
    #[wasm_bindgen]
    pub fn get_bundled_stubs_count(&self) -> usize {
        BUNDLED_STUBS.len()
    }
}
...
impl LS {
    ...
    fn completion(&mut self, id: Option<Value>, p: Option<Value>) -> Option<String> {
        ...
        let items: Vec<CompletionItem> = doc
            .complete(
                InputPosition::Utf16CodeUnits { line, column: col },
                true,
                |_, c: &dyn zuban_python::Completion| {
                    if module
                        .as_ref()
                        .and_then(|m| unavail.get(m))
                        .map(|s| s.contains(c.label()))
                        .unwrap_or(false)
                    {
                        return None;
                    }
    ...
    fn hover(&mut self, id: Option<Value>, p: Option<Value>) -> Option<String> {
        ...
        let h = self.proj.document(&self.path()).and_then(|doc| {
            match doc.documentation(input, false) {
                Ok(Some(r)) => {
        ...
        self.ok(
            id,
            h.map(|v| serde_json::to_value(v).unwrap())
    ...
    fn goto(&mut self, id: Option<Value>, p: Option<Value>, goal: GotoGoal) -> Option<String> {
        ...
        let locs: Vec<Location> = doc
            .goto(input, goal, true, |n| loc(&uri, n))
            .unwrap_or_default();
        self.ok(id, locs)
    }
    ...
    fn references(&mut self, id: Option<Value>, p: Option<Value>) -> Option<String> {
        ...
        let incl = p
            .as_ref()
            .and_then(|v| v.get("context")?.get("includeDeclaration")?.as_bool())
            .unwrap_or(true);
        let uri = self.uri.clone();
        let doc = self.proj.document(&self.path())?;
        let locs: Vec<Location> = doc
            .references(input, ReferencesGoal::OnlyCurrentFile, incl, |n| {
                loc(&uri, n)
            })
            .unwrap_or_default();
        self.ok(id, locs)
    ...
    fn sig_help(&mut self, id: Option<Value>, p: Option<Value>) -> Option<String> {
        ...
        let res = match doc.call_signatures(input) {
            Ok(Some(sigs)) => {
                ...
                let sigs: Vec<SignatureInformation> = sigs
                    .into_iterator()
                    .enumerate()
                    .map(|(i, s)| {
                        ...
                        let params = s.params().map(|ps| {
                            ps.map(|p| ParameterInformation {
                                label: ParameterLabel::Simple(p.name().into()),
                                documentation: None,
                            })
                            .collect()
                        });
                ...
                serde_json::to_value(SignatureHelp {
                    signatures: sigs,
                    active_signature: Some(act_sig),
                    active_parameter: act_param,
                })
                .unwrap()
            }
            _ => Value::Null,
    ...
    fn rename(&mut self, id: Option<Value>, p: Option<Value>) -> Option<String> {
        ...
        if new.is_empty() {
            return Some(self.err(id, -32600, "New name required"));
        }
            ...
            Ok(chg) => {
                ...
                for fc in chg.changes {
                    ...
                    edits
                        .entry(u)
                        .or_default()
                        .extend(fc.ranges.iter().map(|(s, e)| TextEdit {
                            range: range!(s, e),
                            new_text: new.clone(),
                        }));
                }
    ...
    fn doc_symbol(&mut self, id: Option<Value>) -> Option<String> {
        let doc = self.proj.document(&self.path())?;
        #[allow(deprecated)]
        let syms: Vec<DocumentSymbol> = doc
            .symbols()
            .map(|s| {
                let n = s.as_name();
        ...
        let hints: Vec<InlayHint> = doc
            ...
            .map(|it| {
                it.map(|h| InlayHint {
                    ...
                    label: InlayHintLabel::String(h.label().into()),
                    kind: Some(h.kind),
                    text_edits: None,
                    tooltip: None,
...
impl LS {
    fn sync(&mut self) {
        ...
        self.proj
            .store_in_memory_file_with_index(self.path(), self.code.clone().into_boxed_str());
    }
    ...
    fn make_diags(&mut self) -> Vec<Diagnostic> {
        let Some(mut doc) = self.proj.document(&self.path()) else {
            return vec![];
        };
        doc.diagnostics()
            .iter()
    ...
    fn path(&self) -> PathWithScheme {
        PathWithScheme::with_file_scheme(self.fs.normalize_unchecked_abs_path(self.fname()))
    }
