...
pub(super) fn request<'a>(req: server::Request) -> Task<'a> {
    ...
    .unwrap_or_else(|err| {
        ...
        show_err_msg!(
            "ZubanLS failed to handle a request from the editor. Check the logs for more details."
        );
...
pub(super) fn notification<'a>(notif: server::Notification) -> Task<'a> {
    ...
    .unwrap_or_else(|err| {
        ...
        show_err_msg!("ZubanLS failed to handle a notification from the editor. Check the logs for more details.");
        Task::nothing()
    })
}
