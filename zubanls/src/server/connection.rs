...
impl ConnectionInitializer {
    ...
    pub(super) fn initialize_start(
        &self,
    ) -> crate::ZResult<(lsp::RequestId, lsp_types::InitializeParams)> {
...
impl Connection {
    ...
    pub(super) fn handle_shutdown(&self, message: &lsp::Message) -> crate::ZResult<bool> {
        match message {
    ...
    /// to joining them.
    pub(super) fn close(self) -> crate::ZResult<()> {
        std::mem::drop(
...
impl ClientSender {
    pub(crate) fn send(&self, msg: lsp::Message) -> crate::ZResult<()> {
