...
use zuban_db::file_revision::FileRevision;
use zuban_db::system::walk_directory::WalkDirectoryBuilder;
use zuban_db::system::{
...
pub(crate) fn url_to_any_system_path(url: &Uri) -> std::result::Result<AnySystemPath, ()> {
    if url.scheme() == "file" {
...
impl LSPSystem {
    ...
    fn make_document_ref(&self, url: Uri) -> Option<DocumentQuery> {
    ...
    fn system_path_to_document_ref(&self, path: &SystemPath) -> Result<Option<DocumentQuery>> {
        let url = Uri::from_file_path(path.as_std_path()).map_err(|()| {
            std::io::Error::new(
    ...
    fn system_virtual_path_to_document_ref(
        ...
        let url = Uri::parse(path.as_str()).map_err(|_| {
            std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
